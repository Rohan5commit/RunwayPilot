...
export function buildFounderReport({
  ...
  const riskSection = topRisks.length > 0
    ? topRisks
        .map(
          (risk, index) =>
            `${index + 1}. ${risk.title} (${risk.metric})\n   ${risk.description}\n   Action: ${risk.recommendation}`
        )
        .join("\n\n")
  ...
  return [
    ...
    "## Executive Summary",
    strategy?.summary ??
      "AI summary pending or NVIDIA NIM not configured. Deterministic financial insights are available below.",
    "",
    "## KPI Snapshot",
    ...
    "",
    "## Board-Ready Note",
    strategy?.board_ready_note ??
      "Fact: the deterministic model identifies current burn, runway, and spend risk. Inference and recommendations become richer when NVIDIA NIM is configured.",
    ""
  ].join("\n");
}
